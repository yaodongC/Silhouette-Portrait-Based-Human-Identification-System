% ÉèÖÃ ImageDatastore µÄ¶Áº¯Êý  
%imds.ReadFcn = @(filename)readAndPreprocessImage(filename);  
  
%%  
function [Iout,pixels,neck,perimeter,compact,irregu] = readAndLabelImage(imset,crop)  
     % crop settings
      rects=crop;  
     % crop settings   
...
% perimeter length
[perimeter(loop),~]=size(outLine);
% compactness
compact(loop)= pixels(loop)/(perimeter(loop))^2;
% irregularity
xM=mean(outLine(:,1));
yM=mean(outLine(:,2));
irregu(loop)=max((outLine(:,1)-xM).^2+(outLine(:,2)-yM).^2)./pixels(loop);
